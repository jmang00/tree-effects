...
# Write header line
f = open(f'sequences/rainbow-glowing.csv','w',encoding='utf-8')
...
# Setup csv writer
f = open(f'sequences/rainbow-glowing.csv','a',encoding='utf-8')
writer = csv.writer(f)
...
# Animation variables
band_width = 4 #the width of the whole hue scale in GIFT coordinates. 1 = bands of colours, 3/4 = every colour is roughly visible once, 6+ = slower fading effect
band_speed = 0.005 # in units/frame
direction = 'in' #in or out, idk why changing the colours differently gives this illusion but it does
movement_speed = 0.010 # in units/frame
no_movements = 50

# Setup
base_hue = 0
coords = np.array(coords)
avg_position = coords.sum(axis=0)/500 # use the average position of the lights as the center
center = copy(avg_position)


for movement in range(no_movements):
    while True:
        target = choice(coords)
        dist = find_dist(center,target)
        if dist > 1:
            break
    
    print(dist)
    movement_frames = int( dist/movement_speed ) #as time=dist/speed
    change = (target - center)/movement_frames
    ...
    for frame in range(movement_frames):
        ...
        for i in range(len(coords)):
            dist = find_dist(coords[i],center)
